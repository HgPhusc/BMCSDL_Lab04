...
UPDATE NHANVIEN 
SET PUBKEY_CLIENT = 'MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCr' -- Bạn có thể thay bằng chuỗi Public Key thật từ code C# của bạn
WHERE PUBKEY_CLIENT IS NULL;

PRINT N'✔ Đã cập nhật PUBKEY_CLIENT mẫu cho các nhân viên cũ!';
GO

-- 1. Thêm cột lưu Private Key vào bảng NHANVIEN
IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('NHANVIEN') AND name = 'PRIVKEY_CLIENT')
BEGIN
    ALTER TABLE NHANVIEN ADD PRIVKEY_CLIENT NVARCHAR(MAX);
END
GO

--Đổi bảng: Lưu thẳng chuỗi văn bản Base64, không dùng Binary nữa
ALTER TABLE NHANVIEN ALTER COLUMN LUONG VARCHAR(MAX);
ALTER TABLE BANGDIEM ALTER COLUMN DIEMTHI VARCHAR(MAX);
GO

-- 2. Cập nhật SP Thêm nhân viên để nhận thêm PRIVKEY
ALTER PROCEDURE SP_INS_PUBLIC_ENCRYPT_NHANVIEN
    @MANV       VARCHAR(20),
    @HOTEN      NVARCHAR(100),
    @EMAIL      VARCHAR(100),
    @LUONG      VARCHAR(MAX),    -- Chỉ nhận chuỗi VARCHAR
    @TENDN      NVARCHAR(100),
    @MK         NVARCHAR(MAX),
    @PUB        NVARCHAR(MAX),
    @PRIV       NVARCHAR(MAX) = NULL
AS
BEGIN
    SET NOCOUNT ON;
    INSERT INTO NHANVIEN (MANV, HOTEN, EMAIL, LUONG, TENDN, MATKHAU, PUBKEY, PUBKEY_CLIENT, PRIVKEY_CLIENT)
    VALUES (@MANV, @HOTEN, @EMAIL, @LUONG, @TENDN, CONVERT(VARBINARY(MAX), @MK, 2), @MANV, @PUB, @PRIV);
END
GO

-- 3. Cập nhật SP Đăng nhập (Trả thẳng chuỗi)
...
BEGIN
    SET NOCOUNT ON;
    SELECT MANV, HOTEN, EMAIL, TENDN, PUBKEY, PUBKEY_CLIENT, ROLE, PRIVKEY_CLIENT,
           LUONG AS LUONG_ENCRYPTED   -- Không cần CONVERT nữa
    FROM NHANVIEN
    WHERE MANV = @MANV AND MATKHAU = CONVERT(VARBINARY(MAX), @MK, 2);
END
GO

-- 4. Cập nhật SP Xem thông tin cá nhân (Trả thẳng chuỗi)
...
BEGIN
    SET NOCOUNT ON;
    SELECT MANV, HOTEN, EMAIL, LUONG, TENDN, PUBKEY, PUBKEY_CLIENT
    FROM NHANVIEN
    WHERE TENDN = @TENDN AND MATKHAU = CONVERT(VARBINARY(MAX), @MK, 2);
END
GO

-- 5. Cập nhật SP Điểm thi (Trả thẳng chuỗi)
ALTER PROCEDURE SP_INS_BANGDIEM_CLIENT
    @MASV           VARCHAR(20),
    @MAHP           VARCHAR(20),
    @DIEMTHI_ENC    VARCHAR(MAX)
AS
BEGIN
    SET NOCOUNT ON;
    IF EXISTS (SELECT 1 FROM BANGDIEM WHERE MASV = @MASV AND MAHP = @MAHP)
        UPDATE BANGDIEM SET DIEMTHI = @DIEMTHI_ENC WHERE MASV = @MASV AND MAHP = @MAHP;
    ELSE
        INSERT INTO BANGDIEM (MASV, MAHP, DIEMTHI) VALUES (@MASV, @MAHP, @DIEMTHI_ENC);
END
GO
...
BEGIN
    SET NOCOUNT ON;
    SELECT bd.MASV, sv.HOTEN AS HOTEN_SV, bd.MAHP, hp.TENHP, bd.DIEMTHI AS DIEMTHI_ENCRYPTED
    FROM BANGDIEM bd
    JOIN SINHVIEN sv ON bd.MASV = sv.MASV
    JOIN HOCPHAN  hp ON bd.MAHP = hp.MAHP
    WHERE bd.MASV = @MASV;
...
SELECT 
    MANV, HOTEN, TENDN, ROLE,
    CONVERT(VARCHAR(MAX), MATKHAU, 2) AS MATKHAU_SHA1,
    CASE WHEN PUBKEY_CLIENT IS NOT NULL THEN N'Có' ELSE N'Chưa có' END AS PUBKEY_STATUS
FROM NHANVIEN;
GO